...
import (
	...
	"github.com/iotaledger/snapshot-tool-new-supply/pkg/hornet/chrysalis"
	"github.com/iotaledger/snapshot-tool-new-supply/pkg/hornet/stardust"
	"github.com/iotaledger/snapshot-tool-new-supply/pkg/hornet/stardust/tpkg"
)
...
func (s *ChrysalisSnapshot) StardustOutputs() ([]iotago3.OutputID, []iotago3.Output) {
	...
	if len(dustOutputsMap) > 0 {
		log.Panic("not all dust outputs could be vacuumed, seems like dust allowance is missing")
	}
...
func main() {
	...
	// clear export folder
	log.Println("clearing existing export folder...")
	_ = os.RemoveAll(cfg.Export.Dir)

	// parse protocol parameters from config into struct
	log.Println("loading protocol parameters ...")
	...
	log.Printf("converted outputs + IDs hash: %s / %s", outputsHash(stardustOutputs)[:10], outputIDsHash(stardustOutputIDs)[:10])
	...
	log.Printf("supply increase outputs + IDs hash: %s / %s", outputsHash(supplyIncreaseOutputs)[:10], outputIDsHash(supplyIncreaseOutputIDs)[:10])
	...
	if cfg.Import.Active {
		log.Printf("generating outputs from %s", cfg.Import.LedgerFile)
	...
	// create one CSV containing all address and balances
	if cfg.Export.Active {
		sumOutputs := append([]iotago3.Output{}, supplyIncreaseOutputs...)
		sumOutputs = append(sumOutputs, csvImportOutputs...)
		log.Printf("writing out summary addresses/balances file to %s", cfg.Export.SummaryFile)
		generateSummaryAddressBalance(cfg, sumOutputs)
	}
	...
	// check if the infos from the chrysalis snapshot file the config
	...
	var nonTreasuryOutputsSupplyTotal, previousLedgerSupplyTotal, supplyIncreaseSupplyTotal uint64
	outputProducerFunc := func() (*stardust.Output, error) {
		...
		if stardustOutputsIndex < len(stardustOutputs) {
			...
			previousLedgerSupplyTotal += output.Deposit()
		...
		if supplyIncreaseOutputsIndex < len(supplyIncreaseOutputs) {
			...
			nonTreasuryOutputsSupplyTotal += output.Deposit()
			supplyIncreaseSupplyTotal += output.Deposit()
	...
	log.Printf("previous supply: %d, supply increase: %d", previousLedgerSupplyTotal, supplyIncreaseSupplyTotal)
	...
	if cfg.ValidateSupply {
		if protoParams.TokenSupply != nonTreasuryOutputsSupplyTotal+treasuryTokens {
			delta := protoParams.TokenSupply - nonTreasuryOutputsSupplyTotal + treasuryTokens
			log.Panicf("supply defined in protocol parameters does not match supply within generated snapshot! %d vs. %d, delta %d", protoParams.TokenSupply, nonTreasuryOutputsSupplyTotal+treasuryTokens, delta)
		}
	}
...
func generateSummaryAddressBalance(cfg *Config, outputs []iotago3.Output) {
	m := map[string]uint64{}

	for _, output := range outputs {
		addr := output.(*iotago3.BasicOutput).
			UnlockConditionSet().Address().
			Address.Bech32(iotago3.NetworkPrefix(cfg.ProtocolParameters.Bech32HRP))
		m[addr] += output.Deposit()
	}

	tuples := make([]AddrBalanceTuple, 0)
	for k, v := range m {
		tuples = append(tuples, AddrBalanceTuple{
			Address: k,
			Tokens:  strconv.FormatUint(v, 10),
		})
	}

	ex := cfg.Export
	writeAddrListCSV(cfg, tuples, path.Join(ex.Dir, ex.SummaryFile.CSV))
	writeAddrListBin(cfg, tuples, path.Join(ex.Dir, ex.SummaryFile.Bin))
}

func generateCSVOutputs(cfg *Config) ([]iotago3.OutputID, []iotago3.Output) {
	csvImportFile, err := os.Open(cfg.Import.LedgerFile)
	if err != nil {
	...
	outputMarker := blake2b.Sum256([]byte(cfg.Import.OutputMarker))
...
func generateNewSupplyOutputs(cfg *Config) ([]iotago3.OutputID, []iotago3.Output) {
	...
	for _, alloc := range cfg.Vesting.Allocations {
		...
		case alloc.Rewards != nil:
			...
			alloc.Addresses = convertAssemblyToIOTA(mustParseUint64(alloc.Rewards.Tokens), asmbTokensTotal, asmbTokensPerAddr, false)
		...
		case alloc.Distribution != nil:
			...
			alloc.Addresses = convertAssemblyToIOTA(mustParseUint64(alloc.Distribution.Tokens), asmbTokensTotal, asmbTokensPerAddr, true)
...
func generateOutputsForGroup(alloc Allocation, cfg *Config, supplyIncreaseMarker []byte,
	...
	targetDir := path.Join(cfg.Export.Dir, alloc.Name)
	...
	for _, addrTuple := range alloc.Addresses {
		...
		deposit += newOutputs[0].Deposit()
		for i, timelock := range timelocks {
			unlockAccumBalance[timelock.UnixTime] += newOutputs[i+1].Deposit()
			deposit += newOutputs[i+1].Deposit()
		}

		outputIDs = append(outputIDs, newOutputIDs...)
		outputs = append(outputs, newOutputs...)

		fileName := fmt.Sprintf("%s-%s-%s.csv", addrTuple.Name, addrTuple.Address, addrTuple.Tokens)
		if len(addrTuple.Name) == 0 {
			fileName = fmt.Sprintf("%s-%s.csv", addrTuple.Address, addrTuple.Tokens)
		}
	...
	log.Printf("generated %d outputs, %d tokens, placed CSVs in %s, outputs + IDs hashes %s / %s", len(outputs), deposit, targetDir, outputsHash(outputs)[:10], outputIDsHash(outputIDs)[:10])
	return outputIDs, outputs
}

func writeAddrListBin(cfg *Config, addrList []AddrBalanceTuple, fileName string) {
	if !cfg.Export.Active {
		return
	}
	...
	for _, tuple := range addrList {
		_, addr, err := iotago3.ParseBech32(tuple.Address)
		if err != nil {
			log.Panicf("unable to convert address from Bech32 address in binary export: %s", err)
		}

		if _, err := f.Write(addr.(*iotago3.Ed25519Address)[:]); err != nil {
			log.Panicf("unable to write address into binary export: %s", err)
		}
		var num [8]byte
		binary.LittleEndian.PutUint64(num[:], mustParseUint64(tuple.Tokens))
		if _, err := f.Write(num[:]); err != nil {
			log.Panicf("unable to write token amount into binary export: %s", err)
		}
	}
}
...
func writeAddrListCSV(cfg *Config, addrList []AddrBalanceTuple, fileName string) {
	...
	header := []string{"Address", "Tokens"}
	if len(addrList[0].OriginAddress) > 0 {
		header = append([]string{"Base58 Address"}, header...)
	}
	...
	sort.Slice(addrList, func(i, j int) bool {
		return mustParseUint64(addrList[i].Tokens) > mustParseUint64(addrList[j].Tokens)
	})

	for _, addr := range addrList {
		row := []string{addr.Address, addr.Tokens}
		if len(addr.OriginAddress) > 0 {
			row = append([]string{addr.OriginAddress}, row...)
		}
...
func convertAssemblyToIOTA(iotaTokensToDistribute uint64, asmbTokensTotal uint64, asmbTokensPerAddr map[string]uint64, injectBase58Addr bool) []AddrBalanceTuple {
	...
	for i := 0; i < len(tuples); i++ {
		if remainder > 0 {
			...
			// so we fill the missing IOTA to the addresses with the highest remainders as long as IOTAs are left.
			tuples[i].iotaRewards += 1
			remainder--
		}
		iotaRewardsControl += tuples[i].iotaRewards

		tuple := AddrBalanceTuple{
			Address: tuples[i].address.Bech32("iota"),
			Tokens:  strconv.FormatUint(tuples[i].iotaRewards, 10),
		}

		if injectBase58Addr {
			address := tuples[i].address
			base58Addr := base58.Encode(append([]byte{0}, address.(*iotago3.Ed25519Address)[:]...))
			tuple.OriginAddress = base58Addr
		}

		addresses = append(addresses, tuple)
	}
